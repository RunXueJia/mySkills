...
- `workspace-task-checklist`
  - 用途：任务开始时判断是否需要持久化任务清单；多步骤、跨文件、长任务、实现加验证、并行子任务或用户要求任务清单时，在当前工作区 `docs/task/` 创建 Markdown 清单。
  - 覆盖：初始化清单、更新单项状态、追加进展日志、标记任务完成，并通过 `docs/task/.current-task-checklist` 记录当前清单。
  - 注意：清单文件命名为 `YYYY-MM-DD-HH-MM-SS-任务名.md`；简单问答、单个命令、很小的单文件改动默认不创建；创建后只在真实状态变化时更新，不为每个细小命令刷写文档。
