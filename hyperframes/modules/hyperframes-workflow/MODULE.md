...
Use local rendering when Docker image build is blocked by network issues, or when the user wants to reuse an already installed host browser:

```powershell
npx hyperframes render --output output.mp4 --no-browser-gpu
```

On Windows, local rendering can avoid repeated Chrome downloads by pointing HyperFrames at the installed browser. Docker containers cannot directly run the Windows host browser, so this only applies without `--docker`:
...
If local render then fails with `FFmpeg not found`, install FFmpeg or add an existing `ffmpeg.exe` directory to `PATH` before rendering. HyperFrames 0.5.7 checks `where ffmpeg`; it does not honor a project-local `FFMPEG_PATH` setting.
...
- Fails after the Docker image is cached with `Browser not available` / `Chrome not found`: inspect the container wrapper and browser env vars. The wrapper should export both `HYPERFRAMES_BROWSER_PATH` and `PRODUCER_HEADLESS_SHELL_PATH` to the installed `chrome-headless-shell` binary.
...
If the user explicitly asks to use the host browser, switch to local render instead of trying to mount Windows Chrome into Docker. Keep a separate `render:docker` script only when deterministic Docker output is still useful.
