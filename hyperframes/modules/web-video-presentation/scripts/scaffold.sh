...
#   bash ./scripts/scaffold.sh ./talk --theme=paper-press
#   bash ./scripts/scaffold.sh --list-themes
#
# 跑完后，看 MODULE.md "Phase 2.4 实现单章" + references/CHAPTER-CRAFT.md
# 了解每章怎么写。卡壳时翻 references/EXAMPLES/ 找完整章节 anchor。
#
# 之后切换主题，覆盖一个文件即可：
#   cp ./themes/<id>/tokens.css \
...
然后：
  ...
  • 把 src/chapters/01-example/ 替换成你自己的章节
    （流程见 MODULE.md "Phase 2.4 实现单章" —— 每章一次到位完整版本，
